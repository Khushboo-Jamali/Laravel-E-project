...
if (isset($_POST['up_emp'])) {
    ...
    if (move_uploaded_file($tmp_name, $folder)) {
        $sql = "UPDATE `employees` SET `first_name`='$fname',`last_name`='$lname',`email`='$email',`phone`='$phone',`pic`='$folder' WHERE employee_id ='$id'";
        $qurey = mysqli_query($conn, $sql);
        if ($qurey) {
            header("location:user_profile.php");
        }
    } else {
        $sql = "UPDATE `employees` SET `first_name`='$fname',`last_name`='$lname',`email`='$email',`phone`='$phone' WHERE employee_id ='$id'";
        $qurey = mysqli_query($conn, $sql);
        header("location:user_profile.php");
    }
}
